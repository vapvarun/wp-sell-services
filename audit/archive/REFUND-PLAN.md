...
# PART IX — PORTFOLIO STANDARD: the account layer is plugin-independent

Owner, 2026-07-22: *"set these as standard… account flow should be independent
and each plugin is like service — course, events, services, membership plan,
jobs listing, business listing — each need stripe or woo as payment gateway."*

## 42. The standard

Every Wbcom product sells something and every one needs a gateway:

| Product | Sells |
|---|---|
| Learnomy | courses, memberships |
| Eventonomy | event tickets |
| WP Sell Services | services |
| WP Career Board | job listings |
| Listora | business listings |
| BuddyNext | memberships |

They must NOT each own a private customer identity. **The buyer's billing
identity belongs to the USER, not to whichever plugin took the payment.**

Concretely: a buyer who bought a course last month and a service today types
their address **once**, and both invoices carry it.

## 43. Why WooCommerce's keys make this work

Using Woo's exact meta keys is what makes the layer shared without a shared
library:
...
billing_state       billing_postcode   billing_country
billing_email       billing_phone
billing_gst         ← ours; Woo has no tax-registration field
```

Because they are plain user meta under the de-facto WordPress standard keys:

- any Wbcom plugin reads the address another one captured, with no integration
  code and no cross-plugin dependency;
- a site running WooCommerce contributes its existing addresses for free, and
  inherits ours if Woo is installed later;
- no plugin is required for another to work — drop any of them and the address
  survives on the user.

That is what "independent" has to mean here: **data-level interoperability, not
runtime coupling.** No shared plugin to install, no load-order problem, no
version matrix.

## 44. What that means for implementations

1. **Read and write the Woo keys directly.** Never a private
   `_wpss_billing_*` / `_lrn_billing_*` copy — that forks the address and the
   two drift.
2. **Snapshot on the order/purchase at payment time.** Portfolio-wide rule, for
   the same reason: an invoice must not change when a profile is edited.
3. **Sanitise identically** — country upper-cased ISO-2, email through
   `sanitize_email()` — so a value written by one product is valid for another.
4. **Each plugin may own its own edit UI**, since each has its own account
   area. They edit the same underlying record.
5. **`billing_gst` is the shared tax-registration field** (GSTIN in India, VAT
   ID in the EU). One key, not one per jurisdiction, so an invoice generated by
   any product can show it.

## 45. Reference implementation

`wp-sell-services` `src/functions.php` (commit `a86b867`):
`wpss_get_billing_fields()`, `wpss_get_billing_address()`,
`wpss_save_billing_address()`, `wpss_is_billing_address_complete()`, plus the
`wpss_orders.billing_address` JSON snapshot.

The helpers are prefixed per plugin, which is correct — the *contract* is the
meta keys, not the function names. If duplication across products becomes
