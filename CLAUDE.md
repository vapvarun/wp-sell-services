...
> **READ FIRST (v1.3.1):** [`audit/manifest.json`](audit/manifest.json) is the canonical inventory — 142 REST endpoints (23 controllers + 7 generic in API.php + 1 in Tour.php), 111 AJAX handlers, 18 custom DB tables, 6 Gutenberg blocks, 19 shortcodes, 5 WP-CLI commands, 4 cron jobs, 237 hook firings, 25 admin pages. Use this before grepping. See also [`audit/FEATURE_AUDIT.md`](audit/FEATURE_AUDIT.md), [`audit/FLOW-AUDIT.md`](audit/FLOW-AUDIT.md) (gap analysis — rows marked `NOT AUDITED` mean *unknown*, not *fine*), [`audit/ROLE_MATRIX.md`](audit/ROLE_MATRIX.md), [`audit/journeys/`](audit/journeys/) (browser walks; run these before handing anything to QA), [`audit/wppqa-baseline-2026-06-06/SUMMARY.md`](audit/wppqa-baseline-2026-06-06/SUMMARY.md). *(Corrected 2026-08-17: this line previously pointed at `audit/CODE_FLOWS.md`, which has never existed — it now points at the flow doc that does. `ROLE_MATRIX.md` was also missing and has been written from live role data.)* Refresh via `/wp-plugin-onboard --refresh` after non-trivial changes. **manifest_refresh: agent-enumeration-only** — REST is registered through a controller-array wrapper (`src/API/API.php`), so the deterministic generator (`write-manifest.mjs`) undercounts REST (142 → 7) and will silently clobber this manifest. Do NOT run the generator against this plugin; refresh categories by agent enumeration only.
...
- **Pro Version**: WooCommerce, EDD and FluentCart integrations, Stripe/PayPal/Razorpay payments, push notifications, advanced analytics, wallet integrations, earnings ledger + CSV export. (SureCart was removed in 1.6.0: a hosted catalogue cannot act as a payment rail the way WooCommerce does.)
