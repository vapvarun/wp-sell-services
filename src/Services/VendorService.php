...
class VendorService {
	...
	public function grant_vendor_access( int $user_id ): bool {
		...
		// The approval email is NOT sent here. set_status() fires
		// wpss_vendor_status_updated straight after calling this method, and the
		// listener on it runs NotificationService::notify_vendor_approved(),
		// which renders the same vendor-approved.php through EmailService and
		// also writes the in-app notification. Sending from here as well put two
		// copies of one email in the vendor's inbox, under two different
		// subjects, for a single click. See Basecamp 10304014441.
	...
	public function revoke_vendor_access( int $user_id ): bool {
		...
		// The rejection email is NOT sent here either, for the same reason as the
		// approval one in grant_vendor_access(). The listener on
		// wpss_vendor_status_updated already distinguishes rejected from
		// suspended and sends the matching notification for each, so the
		// status check this method used to do is covered there too.
		// See Basecamp 10304031125.
