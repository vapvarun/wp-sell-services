module.exports = function ( grunt ) {
	...
	grunt.registerTask( 'verify:smoke-gate', 'Assert a smoke pass exists for this version', function () {
		 ...
		 *
		 * This read a single path, so free-mode coverage was optional in
		 * practice - and .last-smoke-pass-free.json had never existed, meaning
		 * every release so far shipped with the free-only experience never
		 * exercised in a browser. That matters more here than for most
		 * plugins: Pro changes behaviour on SHARED surfaces (white-label
		 * branding, payout rails, the four raised wizard limits, tiered
		 * commission), so "works with Pro active" is not evidence about the
		 * free product, which is what most installs run.
		 */
		const reports = [
		 ...
		 *
		 * Docs, tests and audit manifests do not: re-running a 40-minute walk
		 * because a comment changed is how a gate gets bypassed rather than
		 * satisfied.
		 */
		...
		for ( const [ mode, report ] of reports ) {
			if ( ! fs.existsSync( report ) ) {
				grunt.fail.warn(
					'verify:smoke-gate — no ' + report + ' (' + mode + ' mode).\n' +
			...
			if ( data.release_version !== pkg.version ) {
				grunt.fail.warn(
					'verify:smoke-gate — ' + mode + ' smoke evidence is for ' +
			 ...
			 *
			 * It compares '1.7.2' to '1.7.2' and stays true for the whole
			 * cycle, so a walk from the FIRST commit of a release certifies
			 * the last one. On 2026-09-23 the report on file was four days and
			 * ~50 commits old - covering changes to service creation, the
			 * Service model and add-on persistence - and this gate read green.
			 *
			 * So the walk records the SHA it ran against, and we ask git what
			 * has changed in the rendered surfaces since.
			 */
			/*
			 * Validate before it reaches git, and never through a shell.
			 *
			 * This value comes out of a JSON file written by the smoke walk -
			 * an agent, not a deterministic tool - so it is untrusted input by
			 * construction. Interpolating it into a shell string was a command
			 * injection: a report carrying `x; rm -rf ~ ;` would have run it.
			 *
			 * Two defences, because either alone is thin. The regex rejects
			 * anything that is not a hex SHA, which also stops argument
			 * injection via a leading '-' (`--upload-pack=...` is a real git
			 * attack). execFileSync passes the value as one argv element, so
			 * no shell ever parses it even if the regex is later loosened.
			 */
			if ( ! /^[0-9a-f]{7,40}$/.test( String( data.ran_against || '' ) ) ) {
				grunt.fail.warn(
					'verify:smoke-gate — ' + report + ' has no usable "ran_against" commit SHA' +
			...
			try {
				...
				grunt.fail.warn(
					'verify:smoke-gate — cannot diff ' + data.ran_against + '..HEAD (' + mode + ').\n' +
			...
			if ( stale.length ) {
				grunt.fail.warn(
					'verify:smoke-gate — the ' + mode + ' walk ran at ' + data.ran_against.slice( 0, 8 ) +
			...
			if ( unrun.length ) {
				grunt.fail.warn(
					'verify:smoke-gate — ' + mode + ': ' + unrun.length + ' section(s) never ran: ' + unrun.join( ', ' ) +
			...
			if ( totals.skipped > totals.pass ) {
				grunt.fail.warn(
					'verify:smoke-gate — the ' + mode + ' smoke skipped more than it verified (' +
		...
		grunt.log.ok(
			'smoke pass on file for ' + pkg.version + ' — both combo and free walks, ' +
