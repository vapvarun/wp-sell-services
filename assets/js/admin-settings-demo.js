...
( function( $ ) {
	...
	$( function() {
		...
		$( '.wpss-import-demo' ).on( 'click', function() {
			...
			window.wpssConfirm( l10n.confirmImport || 'Import demo content? This will create sample services, vendors, and categories.' ).then( function( ok ) {
				if ( ! ok ) {
					return;
				}
		...
		$( '.wpss-delete-demo' ).on( 'click', function() {
			...
			window.wpssConfirm( l10n.confirmDelete || 'Delete all demo content? This will permanently remove demo services, vendors, and empty categories.', { tone: 'danger' } ).then( function( ok ) {
				if ( ! ok ) {
					return;
				}
