...
> **READ FIRST (v1.7.0, released 2026-09-01):** [`audit/manifest.json`](audit/manifest.json) is the canonical inventory. Counts re-measured 2026-09-01 at the 1.7.0 release gate: **209 PHP files in `src/`**, **152 `register_rest_route` calls across 26 controllers**, **121 AJAX handlers**, **20 custom DB tables**, **6 Gutenberg blocks**, **20 shortcodes**, **9 WP-CLI command files**, **417 hook firings (184 do_action + 233 apply_filters)**, **37 HTML email templates** (the 14 unloaded plain-text ones were removed in 1.8.0), **24 switchable notification types**. Use this before grepping. See also [`audit/FEATURE_AUDIT.md`](audit/FEATURE_AUDIT.md), [`audit/FLOW-AUDIT.md`](audit/FLOW-AUDIT.md) (gap analysis — rows marked `NOT AUDITED` mean *unknown*, not *fine*), [`audit/ROLE_MATRIX.md`](audit/ROLE_MATRIX.md), [`audit/journeys/`](audit/journeys/) (browser walks; run these before handing anything to QA). **manifest_refresh: agent-enumeration-only** — REST is registered through a controller-array wrapper (`src/API/API.php`), so the deterministic generator (`write-manifest.mjs`) undercounts REST and will silently clobber this manifest. Do NOT run the generator against this plugin; refresh categories by agent enumeration only. *(Done in 1.7.0: `bin/generate-hook-reference.py` writes the hook reference from source and `bin/docs-audit.py` fails when it is stale, so hook citations no longer rot between releases. `bin/gen-openapi.php` does the same for the REST catalogue.)*
...
|---|---|---|
| 2026-09-01 | **1.7.0 RELEASED** | **Tagged `499177c` (Free) / `4703d7b` (Pro), both on GitHub with zips, lockstep.** The release itself is in the changelog; what belongs here is what the release process found. **Five gates existed and none of them worked.** PHPStan had been aborting before analysing anything since the dead-code sweep, because the baseline still listed `FieldRenderer.php` and `MigrationManager.php`; with it running again it found a **real fatal** - `Dispute::get_resolution_label()` called `self::get_resolution_types()`, a method that lives on `DisputeService`, so the accessor fataled on any resolved dispute and survived only because nothing calls it. `npm run lint` had **never run**: both linters installed and wired into package.json with no config file, so every invocation died with ConfigurationError - configured for correctness it found a dead `border-bottom: none` overridden two lines later. `wp wpss preflight` was **red by construction**: it failed on any log line matching `wpss`, and the contract suite deliberately drives guards into refusing, so it could never pass on a machine a release is cut from. Gate E (REST reachability + OpenAPI catalogue) had no tooling at all - ported and now 186 routes / 0 without a permission callback / 0 GET 5xx. Gate C had no `qa-config.json` or runbook, so the browser smoke could not run and the newest evidence was three weeks old. **The lesson: a gate that reports nothing is indistinguishable from a gate that passes.** Every one of these looked installed. **Two new gates**, both proven to bite before being trusted: `verify:smoke-gate` runs first in `grunt release` and refuses to build on a stale or failing smoke report, and Pro finally has a pre-commit hook - the 1.7.0 build left **1011 dev-package references** in its tracked autoloader, which would have fataled a fresh clone, and Pro had nothing to stop it. My first two versions of that hook silently approved the very thing they existed to block: a case-sensitive pattern against lowercase vendor paths, then `git diff | grep -q` under `set -o pipefail`, where grep exits on match, git takes SIGPIPE, and the pipeline reports failure - so a match read as no-match. **Test that a gate fails, never that it passes.** Also: `grunt dist` is the dev path and skips `composer:nodev` and the autoloader check - `grunt release` is the only task that cuts a shippable zip; and Pro was shipping `bin/` because `.distignore` excluded it but `copy:dist` did not, two exclude lists disagreeing. |
| 2026-08-26 | 1.7.0 (dev) | **Full board sweep: 36 cards, plus a docs audit and a dead-code sweep.** The shape this round was **one question answered in more than one place**. *Money REST surface*: eight private copies of "is this caller a vendor" had drifted - three different denial codes, reads login-only while writes were vendor-gated, and `wpss_rest_require_vendor()` never consulted `wpss_vendor_status_block()` so a **suspended vendor kept full access everywhere**. Now one gate on `RestController` delegating to the canonical helper; `tests/test-money-route-gates.php` asserts the whole surface and that no route carries two permission callbacks. *Emails*: the 23 notification types were written **twice** in `Settings.php` and had drifted - `notify_moderation` was gated on by EmailService and written by nothing, and `TYPE_REVIEW_RECEIVED` was missing from the map so unchecking "New Review" silenced one path and not the other. One `get_notification_types()` now, 24 types, and `tests/test-notification-toggle-contract.php` asserts both directions. *Privacy*: registered a real exporter + eraser (orders, messages, reviews, profile, ledger); erasure delegates to `AccountDeletionService` so obligations are honoured. My first version queried three columns that do not exist and reported a clean empty export - it now logs on query error. *Dashboard*: `wpss_append_dashboard_section()` dropped `orders` as "the default", which stopped being true in 1.2.0 when landing became role-aware - a dual-role member clicking My Orders landed on Sales Orders. *Cart link* derives from the rail that owns checkout instead of a checkbox that was off by default. *Offline payments* gained named methods, frozen onto the order in `meta` so a rename cannot rewrite an old receipt - `payment_method` is deliberately untouched because live data carries both `offline` and `bacs`. *Message emails* can be held N minutes and re-checked at fire time; nothing cancels the job, it just no-ops. **Dead-code sweep: -1,299 lines** - `MigrationManager` (543, orphaned second migration system), `FieldRenderer` (203), four zero-implementation interfaces, Pro's duplicate `PaymentGatewayInterface`, the six wizard feature flags and their unused reader, and five uncalled methods. **RecurringServices deliberately kept** - 2,820 unreachable lines, but parked work behind a documented kill switch, not slop. **Docs**: 107 pages audited against source; ~75% carried at least one claim the code contradicted. Removed a manual-refund instruction that caused double refunds, a false GDPR assurance, a "files are private" claim, inverted access-control instructions, and four Pro features that do not ship. |
| 2026-08-18 | **1.6.0** | **Bug sweep closed, then the API and mobile auth cards.** The recurring shape this round was **a store with a reader and no writer, or the reverse**: offline pay-order never wrote `payment_method` (so the order could be confirmed by nobody), push tokens were stored since 1.1.0 and never read, the unread badge was counted correctly then hidden by `text-indent: -999999px`. Also **`get_post( 0 )` returns the global `$post`**, which is why proposal orders resolved their service to whichever page was rendering. **API normalised** (Basecamp 10154919636): one actor shape via `wpss_rest_user()`, one date format (12 emissions were bare MySQL with no timezone; DisputesController carried a third implementation of its own), one card shape via new `wpss_rest_service_card()` - `/favorites` could not reuse `/services`' builder because its pieces were private methods on the controller. All additive, so `contract_version` stays at 1. **Mobile auth hardened** (10154918753): app tokens now expire (idle 30d / absolute 90d) enforced at authentication by `API\AppTokenGuard`, and a token can no longer be passed as the account password to mint more tokens - both reproduced over HTTP first. `GET` / `DELETE /auth/sessions` added, deliberately NOT on `/auth/devices`, which already means push tokens. **Two traps worth remembering:** `rest_do_request()` skips `rest_post_dispatch`, so `_fields` and app-password auth both behave differently under `wp eval-file` than over HTTP - it produced two false negatives this session; and WP 6.8 moved app passwords onto fast hashes, so `wp_check_password()` returns false for a valid token and any guard built on it fails OPEN.  **Second half of the day, six more cards.** **Checkout was full-bleed on any theme that opens its content wrapper in a page template rather than `header.php`** - reproduced on stock Twenty Twenty-Four, so not a BuddyX Pro quirk; now wraps in `.wpss-container`, and `WPSS.relaxRedundantContainer()` stands that down where the theme already constrains (owner decision: the theme's width wins). **Service gallery** opens on the image, video leads the thumb strip with the PROVIDER's own poster via `wpss_get_video_thumbnail_url()` (cached a week), the embed lives in a `<template>` so YouTube is never contacted until asked for, and the template no longer ran `update_post_meta()` on every render. **Category choosers**: wizard offers parents only - note the obvious fix (`parent => 0` on the fetch) would have emptied the Subcategory dropdown, because the same fetch feeds `window.wpssCategories`; five other surfaces grouped via new `wpss_group_category_terms()`. **Wizard checklist** icons: `x-effect` failed three ways at once, including the `or` chain SHORT-CIRCUITING so only `data.title` was ever a dependency; replaced with two static icons toggled by `x-show`. **`wpss_member_bypasses_limits()`** - the site owner was blocked by their own paywall, and there are TWO gates on `wpss_vendor_can_create_service`, so test through the FILTER not the class. **Demo seeder** now builds a category hierarchy AND files services in it - demo data is test coverage, and the subcategory surface had shipped untested because no seeded child term existed. **`wp wpss api:shapes`** committed (`src/CLI/ApiShapeCommand.php`): walks every GET route, fills parameterised routes from real rows, fails on a MySQL date or an actor missing `deleted`. It found two emissions that a hand-rolled HTTP sweep AND QA's independent 23-route sweep had both missed. Four new journeys (05-08) and the REST payload contract documented. |
| 2026-08-13 | **1.6.0** | **Bug-fix release; the recurring root cause was one flow implemented twice.** Nearly every card resolved to duplicated logic that had drifted: three disagreeing rail status maps → one `wpss_map_rail_status()`; two member dashboards → one (`[wpss_account]` is now a wrapper); two admin gates; two brand accents; two message renderers; two meanings for `package_id`. (1) **Package identity frozen on every order** — `wpss_assign_package_ids()` gives packages a stable id (base 1000 via `wpss_package_id_base`, deliberately above any legacy index so the two can never be confused), `wpss_resolve_service_package()` reads stable-id-first with an index fallback, and `backfill_package_snapshots()`/`backfill_package_ids()` repair existing orders against what the buyer actually paid. (2) **Offline payment receipts, end to end** — new `wpss_payment_receipts` table + `PaymentReceiptRepository`/`PaymentReceiptService`; buyer uploads proof, admin verifies, verification routes through `StandaloneOrderProvider::mark_as_paid()` (the single credit path, never a second one). Claim is row-locked via a conditional `UPDATE … WHERE status='submitted'`, so two admins approving at once credits once. Off by default. 3 new email types + printable receipt. (3) **Role-aware email preferences** — `wpss_get_email_preference_categories()` (buyer 5, vendor 8); `save_email_preferences()` now derives its allowlist from that registry rather than a hardcoded 8-key list, which would have silently muted every buyer's tips/withdrawals/proposals. (4) **Checkout billing fields configurable**; first/last name + email locked. (5) **i18n gate hardened** — `bin/i18n-verify.py` (identical copy in both plugins, config-driven) now fails on version drift between the plugin header, POT `Project-Id-Version`, `package.json` and readme Stable tag; 54 dead English JS fallbacks removed. Schema `DB_VERSION` 1.5.3 → 1.6.0 (`platform_order_ref` + `idx_platform_ref`, package snapshots, package ids, `payment_receipts`). |
...
## Key Features (current: 1.7.0, released)
...
- **`wpss_pay_order_url` seam** — the single way to send a buyer to pay one existing order. WooCommerce implements it (`WCPayOrderResolver`, creates/reuses a real WC order); EDD does not, so milestone/tip/extension pay links are a dead end there. Known gap, documented. (FluentCart gained the seam in 1.6.0; SureCart was removed.)
...
**`grunt release`, never `grunt dist`.** `dist` is the quick dev rebuild: it
skips `composer:nodev` and `verify:dist-autoloader`, so the zip it produces
carries an autoloader referencing PHPUnit and PHPStan and fatals on activation.
`release` is the only task that cuts a shippable archive.

```bash
./node_modules/.bin/grunt release     # both plugins; smoke-gated, autoloader-gated
```

**After any release build, restore the tracked autoloader.** `release` ends with
`composer:dev` to give you your tools back, which regenerates
`vendor/composer/` *with* dev packages — and that directory is committed,
because the runtime vendor ships in the repo. Committing it fatals a fresh
clone. Both plugins have a pre-commit hook that blocks this; the fix is
`composer dump-autoload --no-dev`, stage, then `composer dump-autoload` to get
your tools back.

### The gates, and why each exists

| Gate | Command | Guards against |
|---|---|---|
| Lint + PHPStan | `php vendor/bin/phpstan analyse` | type and namespace bugs; found a real fatal in 1.7.0 |
| Contract audit | `contract-audit.php --pair=…` | key/hook contract drift between Free and Pro |
| Browser smoke | `wp-plugin-smoke` skill | rendered behaviour; writes `docs/qa/.last-smoke-pass.json` |
| Docs truth | `bin/docs-audit.py` | docs contradicting the code being shipped |
| REST | `tests/audit/rest-reachability.php` | an unguarded or fataling endpoint anywhere |
| Catalogue | `bin/gen-openapi.php` | the published API spec going stale |
| Preflight | `wp wpss preflight` | install health |
| Pristine install | Docker, from the built zip | packaging faults no dev-tree gate can see |

**PHPStan needs a dev autoload shim.** The shipped `vendor/autoload.php` is
generated `--no-dev`, so `szepeviktor/phpstan-wordpress` is present on disk but
unmapped and PHPStan aborts. Pass `--autoload-file` with a shim registering that
namespace. Pro carries no phpstan of its own by design — run it with Free's
binary and a config whose include points at Free's `extension.neon`.

**Never publish a zip you have not installed fresh**, and if you rebuild after
testing, test again — the artifact that ships must be the artifact that was
installed. Verify by downloading the release asset back and comparing sha256.

### What this codebase teaches about gates

Five gates were found broken during the 1.7.0 release, every one of them
looking installed and checking nothing: PHPStan aborting on a stale baseline,
`npm run lint` with no config file, `wp wpss preflight` red by construction,
Gate E with no tooling, Gate C with no runbook.

**A gate that reports nothing is indistinguishable from a gate that passes.**
So when you add or change one, prove it *fails* on the thing it exists to
catch — asserting that it passes proves nothing. Two versions of the Pro
pre-commit hook silently approved the exact condition they were written to
block, and only a deliberate red test found it.
...
| Standalone adapter + Offline gateway | EDD/FluentCart adapters |
