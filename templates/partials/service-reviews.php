				...
				<div class="wpss-review">
					...
					<div class="wpss-review-helpful">
						<?php if ( is_user_logged_in() && (int) ( $review->reviewer_id ?? 0 ) !== get_current_user_id() ) : ?>
							<button type="button" class="wpss-helpful-btn" data-review-id="<?php echo esc_attr( $review->id ); ?>" style="background:none;border:1px solid #ddd;border-radius:4px;padding:4px 10px;cursor:pointer;font-size:13px;color:#666;display:inline-flex;align-items:center;gap:4px;">
								...
								<?php esc_html_e( 'Helpful', 'wp-sell-services' ); ?>
							</button>
						<?php endif; ?>
						<?php if ( isset( $review->helpful_count ) && $review->helpful_count > 0 ) : ?>
							<span class="wpss-helpful-count" style="color:#999;font-size:13px;">
								<?php
								printf(
