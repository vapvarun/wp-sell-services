...
class ConversationService {
	...
	public function create_for_order( int $order_id ): ?Conversation {
		...
		$participants = [ $order->customer_id, $order->vendor_id ];
	 ...
	 * @return Message[]
	 */
	public function get_messages( int $conversation_id, array $args = [] ): array {
		global $wpdb;
	...
	public function send_message(
		...
		$wpdb->insert(
			...
			[
				...
				'read_by'         => wp_json_encode( [ $sender_id => true ] ),
				'is_edited'       => 0,
				'created_at'      => current_time( 'mysql' ),
				'updated_at'      => current_time( 'mysql' ),
			],
	 ...
	 * @return Message|null
	 */
	public function add_system_message( int $conversation_id, string $content, array $metadata = [] ): ?Message {
		$conversation = $this->get( $conversation_id );
	...
	public function mark_as_read( int $conversation_id, int $user_id ): bool {
		...
		foreach ( $unread_messages as $msg ) {
			...
			$wpdb->update(
				$messages_table,
				[ 'read_by' => wp_json_encode( $read_by ) ],
				[ 'id' => $msg->id ]
			);
		}
