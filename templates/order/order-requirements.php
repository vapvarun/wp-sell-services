...
if ( ! in_array( $order->status, [ 'pending_requirements', 'pending_payment' ], true ) ) {
...
<div class="wpss-requirements-page">
	<div class="wpss-requirements-header">
		<h1><?php esc_html_e( 'Submit Requirements', 'wp-sell-services' ); ?></h1>
	...
	<div class="wpss-requirements-layout">
		<div class="wpss-requirements-main">
			<!-- Order Summary -->
			<div class="wpss-order-summary-card">
				<div class="wpss-service-info">
					...
					<div class="wpss-service-details">
						<h3><?php echo esc_html( $service ? $service->post_title : __( 'Service', 'wp-sell-services' ) ); ?></h3>
						<p class="wpss-order-number">
							<?php
							printf(
				...
				<?php if ( ! empty( $requirements ) ) : ?>
					<?php foreach ( $requirements as $index => $requirement ) : ?>
						<div class="wpss-requirement-field">
							<label for="requirement_<?php echo esc_attr( $index ); ?>">
							...
							<?php elseif ( 'file' === ( $requirement['type'] ?? 'text' ) ) : ?>
								<div class="wpss-file-upload-field">
									<input type="file"
										   id="<?php echo esc_attr( $field_id ); ?>"
										   name="<?php echo esc_attr( $field_name ); ?>"
										   <?php echo ! empty( $requirement['required'] ) ? 'required' : ''; ?>
							...
							<?php if ( ! empty( $requirement['description'] ) ) : ?>
								<p class="wpss-field-hint"><?php echo esc_html( $requirement['description'] ); ?></p>
							<?php endif; ?>
						</div>
					<?php endforeach; ?>
				<?php else : ?>
					...
					<div class="wpss-requirement-field">
						<label for="requirements_description">
		...
		<aside class="wpss-requirements-sidebar">
			<!-- Order Details -->
			...
			<?php if ( $vendor ) : ?>
				<div class="wpss-sidebar-card">
					<h4><?php esc_html_e( 'Your Seller', 'wp-sell-services' ); ?></h4>
					<div class="wpss-user-card">
						<img src="<?php echo esc_url( get_avatar_url( $vendor->ID, [ 'size' => 60 ] ) ); ?>"
			...
			<!-- Help Box -->
			<div class="wpss-sidebar-card wpss-help-box">
				<h4><?php esc_html_e( 'Need Help?', 'wp-sell-services' ); ?></h4>
