...
| Frontend party dispute messaging | ✅ | **built the missing surface.** Backend (`wpss_add_dispute_evidence` AJAX + `DisputeService::add_evidence`) existed but no frontend rendered the thread or a reply form — a party could open a dispute then never respond. Wired thread + add-message/attach-file form into the detail view; JS in frontend.js/.min.js; premium chat-bubble CSS (own=indigo/right, other=gray/left) in all 4 unified-dashboard CSS files. Verified two-way: Oliver posts → Diego sees as "other" + replies → both persist (`wpss_disputes.evidence` JSON) and render server-side. **Fixed 2 latent bugs surfaced by wiring it:** (1) fatal `find('27')` TypeError — order_id string passed to `find(int)`, now cast; (2) `add_evidence()` did `absint()` on uploaded file URLs → stored 0, now `esc_url_raw()` |
...
| PayPal payout double-pay fix + all-rails ledger settlement | ✅ **FIXED + verified 2026-07-26 (pro @ 375e670).** Double-pay root cause: PayPalPayouts computed owed from `SUM(orders.vendor_earnings) WHERE payment_status != 'paid_out'` — but nothing ever wrote `paid_out` AND no ledger debit was written, so every batch re-paid lifetime earnings. Rebuilt onto the ledger: `paypal_payout` registered as a debit type; `get_pending_payouts` = `wpss_get_ledger_balance` − in-flight reserve; `record_payout_debit` (idempotent, keyed on payout item) on batch-item SUCCESS. **All 3 rails now settle on the one SUM-authoritative ledger + owed-from-ledger** — Manual (`withdrawal`, verified 148.5→98.5), Stripe Connect (`connect_transfer`, ConnectLedgerBridge active), PayPal (`paypal_payout`, verified owed→reserve→debit→no re-pay→idempotent). Zero-integration manual path (Mark-paid + export-owed CSV) intact. **Remaining (creds-gated, NOT accounting):** live PayPal Payouts sandbox round-trip + refund-after-payout debt-netting on the PayPal rail |
